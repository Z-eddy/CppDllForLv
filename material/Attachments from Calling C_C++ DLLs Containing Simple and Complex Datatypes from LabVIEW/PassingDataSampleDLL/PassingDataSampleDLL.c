...
{
	printf("\n\nCalling Function: %s \nDescription: %s", functionName, functionDescription);
}
...
{	
    int i;
    *newLength = length + 1;
    *newArray = realloc(*newArray, sizeof(int) * *newLength);
    for (i=0; i<length; i++)
        (*newArray)[i] = x[i];
    (*newArray)[i] = -1;
}
...
{
	*newString = malloc(sizeof(char) * (strlen(str) + 3));
...
{
	int i;
	*circleArray = malloc(sizeof(struct simpleStructCircle) * length);
	for (i=0; i<length; i++)
	{
...
struct complexStructPolygon ReturningAValue_ComplexStruct (void)
{
	...
	triangle.vertices = malloc (sizeof (struct point) * triangle.numVertices);
...
{
	struct complexStructPolygon *pTriangle = malloc (sizeof(struct complexStructPolygon));
...
{
	int i;
	*triangles = malloc(sizeof(struct complexStructPolygon) * length);
	for (i=0; i<length; i++)
	{
